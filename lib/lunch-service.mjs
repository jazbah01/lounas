...
export async function getLunchData(now = new Date()) {
  ...
  const results = await Promise.all(
    restaurants.map(async (restaurant) => {
      try {
        const source = await fetchRestaurantSource(restaurant, now);
        const items = parseRestaurantMenu(restaurant.parser, source, now);
...
async function fetchRestaurantSource(restaurant, now) {
  if (restaurant.parser !== "mealdoo") {
    return fetchSource(restaurant.url);
  }

  const sourceUrl = new URL(restaurant.sourceUrl);
  sourceUrl.searchParams.set("menu", restaurant.menu);
  sourceUrl.searchParams.set("dates", formatApiDate(now));

  return fetchSource(sourceUrl);
}
...
function parseRestaurantMenu(parser, source, now) {
  switch (parser) {
    case "patakunkku":
      return parseWeeklyDaySections(source, now, {
    ...
    case "nekalanhovi":
      return parseNekalanhovi(source, now);
    case "mealdoo":
      return parseMealdoo(source);
    case "zafran":
      return parseZafran(source, now);
    case "evas":
      return parseEvas(source, now);
    case "stahlberg":
      return parseStahlberg(source, now);
    default:
...
function parseMealdoo(source) {
  let response;

  try {
    response = JSON.parse(source);
  } catch {
    throw new Error("Mealdoon vastaus ei ollut kelvollista JSON-dataa.");
  }

  const day = Array.isArray(response) ? response[0] : null;
  if (!day?.allSuccessful || !day?.isMenuForDate || !day.data?.mealOptions) {
    return [];
  }

  return day.data.mealOptions
    .filter((option) => {
      const name = getLocalizedValue(option.names, "name");
      return !/^aamupuuro$/i.test(name);
    })
    .flatMap((option) => option.rows ?? [])
    .map((row) => {
      const name = getLocalizedValue(row.names, "name");
      const diets = row.diets?.find((diet) => diet.language === "fi")?.dietShorts ?? [];
      return [name, diets.join(", ")].filter(Boolean).join(" ");
    })
    .filter(Boolean);
}

function getLocalizedValue(values, key) {
  return values?.find((value) => value.language === "fi")?.[key] ?? "";
}
